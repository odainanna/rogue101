...
/**
 * TextFont – for grid-based text / character graphics <p> TextFonts are used for drawing text and
 * character graphics. Each character is assumed to be uniform in size, fitting within a square with
 * sides of length {@link #getSquareSize()}. The given font file should contain a monospaced font of
 * a type suitable for JavaFX, such as OpenType or TrueType.
 *
 * <p> Additional horizontal and vertical positioning and scaling can be used to make the font fit
 * with the square-shaped concept.
 *
 * <p> See {@link #setGraphicsContext(GraphicsContext)} for setting up the graphics context for
 * writing with the font, or {@link #setGraphicsContext(GraphicsContext, double)} for extra
 * on-the-fly horizontal scaling, e.g. to make half-width letters ("hires" mode).
 */
public class TextFont {
   ...
   *
   * Each letter should be approximately centered within its available square-shaped space.
   */
  private final double xTranslate;
  /**
   * Vertical positioning of letters.
   *
   * Each letter should be positioned on the baseline so that ascenders and descenders fall within
   * its available square-shaped space.
   */
  private final double yTranslate;
  /**
   * Horizontal scaling factor (1.0 means no scaling)
   *
   * Most fonts are relatively tall and narrow, and need horizontal scaling to fit a square shape.
   */
   ...
   *
   * <p> TextFonts are used for drawing text and character graphics. Each character is assumed to be
   * uniform in size, fitting within a square with sides of length {@link #getSquareSize()}. The
   * given font file should contain a monospaced font of a type suitable for JavaFX, such as
   ...
   * on-the-fly horizontal scaling, e.g. to make half-width letters ("hires" mode).
   *
   * @param font Name of the font file. Will search for the file in the same folder as the TextFont
   * class, as well as ".." and "../fonts".
   * @param size Point size of the font.
   * @param squareSize The width and height of a square defining the bounds of letters
   ...
   */
  public TextFont(Font font, double squareSize, double xTranslate, double yTranslate, double xScale,
      double yScale) {
    super();
   ...
   * @param yScale Vertical scaling factor
   */
  public TextFont(String fileName, double size, double squareSize, double xTranslate,
      double yTranslate,
   ...
   * @param xScale Horizontal scaling factor
   * @param yScale Vertical scaling factor
   * @param deferLoading True if the font file shouldn't be loaded before the font is actually used
   */
  public TextFont(String fileName, double size, double squareSize, double xTranslate,
      double yTranslate,
  ...
  public static void drawMasked(GraphicsContext ctx, Image src, Image mask, double x, double y,
      boolean invert,
      boolean blend) {
  ...
  public static void fillInverse(GraphicsContext ctx, Image img, double x, double y) {
    ...
    int rgb =
        ((int) (c.getRed() * 255)) << 16 | ((int) (c.getGreen() * 255)) << 8 | ((int) (c.getBlue()
            * 255));
   ...
   * Load a font.
   *
   * Will first try to use the provided string as a file name, and load the font from a font file
   * (should be in a format supported by JavaFX). If loading from file fails, we assume we have been
   * given the name of a font which is passed directly to JavaFX for loading. A fallback font is
   * used if this fails.
   *
   * When looking for files, his method will search for the file in a search path starting with the
   * folder containing the {@link TextFont} class file (using Java's standard {@link
   * Class#getResourceAsStream(String)} system). The search path also includes ".." (parent
   * directory) and "../fonts".
   *
   * The loaded font will be cached, so that additional calls with the same file name will not cause
   * the file to be loaded again.
   *
   * If the font cannot be loaded, a default font will be substituted. You may check which font you
   * got using {@link Font#getName()} or {@link Font#getFamily()}.
   *
   * @param name Font name, or relative path to the file
   * @param size Desired point size of the font
   * @return A JavaFX font
   */
  public static Font findFont(String name, double size) {
   ...
   * used if this fails.
   *
   * When looking for files, this method will search for the file in a search path starting with the
   * folder containing the provided Java class (using Java's standard {@link
   * Class#getResourceAsStream(String)} system). The search path also includes ".." (parent
   * directory) and "../fonts".
   *
   * The loaded font will be cached, so that additional calls with the same file name will not cause
   ...
   * @return A JavaFX font
   */
  public static Font findFont(String name, double size, Class<?> clazz) {
    if (name == null || size < 0) {
    ...
    if (font == null) {
      throw new RuntimeException(
          "Even the default font seems to be unavailable – this shouldn't happen! :(");
    }
    if (font.getName().equals(Font.getDefault().getName())) {
      System.err.println(
          "TextFont: Default font '" + font.getName() + "' substituted for '" + name + "'");
    }
  ...
  /**
   * Create a copy of this font, with the given adjustments to the translation and scaling.
   */
  public TextFont adjust(double size, double deltaXTranslate, double deltaYTranslate,
      double deltaXScale,
      double deltaYScale) {
    if (size == 0.0) {
   ...
   *
   * The <code>ctx</code> should normally be translated to the appropriate text position before
   * calling this method.
   *
   * Text will be clipped so each character fits its expected square-shaped area, and the area will
   * be cleared to transparency before drwaing.
   *
   ...
   * @param xScaleFactor a horizontal scaling factor
   */
  public void drawTextAt(GraphicsContext ctx, double x, double y, String text, double xScaleFactor,
      int mode,
      Paint bg) {
    ...
    text.codePoints().forEach((int c) -> {
      ...
      if (c >= 0x2596 && c <= 0x259f) { // quadrants
        ...
        if ((bits & 1) > 0) { // lower right
          ctx.fillRect(squareSize * xScaleFactor / 2, -squareSize / 2,
              (squareSize / 2) * xScaleFactor,
              squareSize / 2);
        }
        ...
        int height = c - 0x2580;
        ctx.fillRect(0, -(height * squareSize) / 8, (squareSize) * xScaleFactor,
            (height * squareSize) / 8);
        ...
        ctx.fillRect(((7 * squareSize) / 8) * xScaleFactor, -squareSize,
            (squareSize / 8) * xScaleFactor,
            squareSize);
      } else if (c == 0x2571) {
        ctx.save();
   ...
   *
   * Caller should call {@link GraphicsContext#save()} first, and then {@link
   * GraphicsContext#restore()} afterwards, to clean up adjustments to the transformation matrix
   * (i.e., translation, scaling).
   *
   * The GraphicsContext should be translated to the coordinates where the text should appear
   * <em>before</em> calling this method, since this method will modify the coordinate system.
   *
   ...
   *
   * @param ctx A GraphicsContext
   * @param xScaleFactor Additional horizontal scaling, normally 0.5 (for half-width characters)
   */
  ...
  private void strokeFullLine(GraphicsContext ctx, double xScaleFactor, double xDir, double yDir,
      char c) {
    ...
    if (c != '.') {
      ...
      if (c == '=') {
        ctx.setLineWidth(2.0);
        ctx.strokeLine((x - 2) * yDir, (y - 2) * xDir,
            (x - 2) * yDir + xDir * squareSize * xScaleFactor,
  ...
  private void strokeHalfLine(GraphicsContext ctx, double xScaleFactor, double xDir, double yDir,
      char c,
      double otherWidth) {
    if (c != '.') {
   ...
   *
   * For most cases, the simpler {@link #drawText(GraphicsContext, String)} or {@link
   * #drawText(GraphicsContext, String, double)} will be easier to use.
   *
   * If <code>clip</code> is true, the graphics context's current path will be overwritten.
   *
   * @param ctx A GraphicsContext
   * @param x X-position of the lower left corner of the text
   * @param y Y-position of the lower left corner of the text
   * @param text The text to be printed
   * @param xScaleFactor Horizontal scaling factor, normally 1.0 (full width) or 0.5 (half width)
   * @param clear True if the area should be cleared (to transparency) before drawing; normally
   * true.
   * @param clip True if the text drawing should be clipped to fit the expected printing area;
   * normally true.
   * @param fill True if the letter shapes should be filled; normally true.
   * @param stroke True if the letter shapes should be stroked (outlined); normally false.
   */
  public void textAt(GraphicsContext ctx, double x, double y, String text, double xScaleFactor,
      boolean clear,
    ...
    if (clear && (mode & ATTR_INVERSE) == 0 && (mode & ATTR_OVERSTRIKE) == 0) {
      ctx.clearRect(0 + 0.5, -squareSize + 0.5, squareSize * width * xScaleFactor - 1,
          squareSize - 1);
    }
