...
public class Screen {
  ...
  /**
   * Screen should start fullscreen, without showing a "Press ESC to exit fullscreen" hint.
   */
   ...
   *
   * The default is {@link #CONFIG_PIXELS_DEVICE} for {@link #CONFIG_SCREEN_FULLSCREEN} and {@link
   * #CONFIG_COORDS_DEVICE}, and {@link #CONFIG_PIXELS_STEP_SCALED} otherwise.
   */
  public static final int CONFIG_PIXELS_DEFAULT = 0 << CONFIG_PIXELS_SHIFT;
  /**
   * Canvas size / number of pixels will be an integer multiple or fraction of the logical canvas
   * size that fits the native display size.
   *
   * Scaling by whole integers makes it less likely that we get artifacts from rounding errors or
   * JavaFX's antialiasing (e.g., fuzzy lines).
   */
  ...
  /**
   * Canvas size / number of pixels will the same as the logical canvas size (typically 1280x960).
   */
  ...
  /**
   * The logical canvas coordinate system will be in logical units (i.e., 1280 pixels wide
   * regardless of how many pixels wide the screen actually is)
   */
  ...
  public Screen(double width, double height, double pixWidth, double pixHeight, double canvasWidth,
    ...
    subScene.layoutBoundsProperty()
        .addListener(
            (ObservableValue<? extends Bounds> observable, Bounds oldBounds, Bounds bounds) -> {
   ...
   *
   * <p> This takes into account such things as toolbars, menus and such (on a desktop), and pixel
   * density (e.g., on high resolution mobile devices).
   *
   ...
   *
   * <p> This will not include such things as toolbars, menus and such (on a desktop), or take pixel
   * density into account (e.g., on high resolution mobile devices).
   *
   ...
   *
   * This will open a window on the screen, and set up background, text and paint layers, and
   * listener to handle keyboard input.
   *
   * @param stage A JavaFX {@link javafx.stage.Stage}, typically obtained from the {@link
   * javafx.application.Application#start(Stage)} method
  ...
  public static Screen startPaintScene(Stage stage, int configuration) {
    ...
    if (debug) {
      ...
      System.out
          .printf("  Display: %.0fx%.0f (raw %.0fx%.0f)%n", width, height, rawWidth, rawHeight);
      System.out.printf("  Window:  %.0fx%.0f%n", winWidth, winHeight);
      System.out.printf("  Canvas:  physical %.0fx%.0f, logical %.0fx%.0f%n", pixWidth, pixHeight,
          canvasWidth,
          canvasHeight);
    ...
    scene.setOnKeyPressed((KeyEvent event) -> {
      ...
      if (!event.isConsumed() && pScene.keyPressedHandler != null && pScene.keyPressedHandler
          .test(event)) {
        event.consume();
      }
    ...
    scene.setOnKeyTyped((KeyEvent event) -> {
      ...
      if (!event.isConsumed() && pScene.keyTypedHandler != null && pScene.keyTypedHandler
          .test(event)) {
        event.consume();
      }
      if (pScene.logKeyEvents) {
    ...
    scene.setOnKeyReleased((KeyEvent event) -> {
      suppressKeyTyped[0] = false;
      if (!event.isConsumed() && pScene.keyReleasedHandler != null && pScene.keyReleasedHandler
          .test(event)) {
        event.consume();
      }
      if (pScene.logKeyEvents) {
  ...
  public boolean isFullScreen() {
    ...
    return window instanceof Stage && ((Stage) window).isFullScreen();
  }
  ...
  public boolean minimalKeyHandler(KeyEvent event) {
    ...
    } else if (!(event.isAltDown() || event.isControlDown() || event.isMetaDown() || event
        .isShiftDown())) {
      if (code == KeyCode.F11) {
  ...
  private void recomputeLayout(boolean resizeWindow) {
    ...
    if (debug) {
      System.out.printf(
          "Rescaling: subscene %1.2fx%1.2f, scale %1.2f, aspect %.4f (%d), canvas %1.0fx%1.0f%n",
          subScene.getWidth(), subScene.getHeight(), currentScale, aspects.get(aspect), aspect,
          getRawWidth(),
          getRawHeight());
    }
    for (Node n : root.getChildren()) {
      n.relocate(Math.floor(subScene.getWidth() / 2),
          Math.floor(
              subScene.getHeight() / 2 + (rawCanvasHeight - getRawHeight()) * currentScale / 2));
      n.setTranslateX(-Math.floor(rawCanvasWidth / 2));
      n.setTranslateY(-Math.floor(rawCanvasHeight / 2));
      if (debug) {
        System.out.printf(" *  layout %1.2fx%1.2f, translate %1.2fx%1.2f%n", n.getLayoutX(),
            n.getLayoutY(),
