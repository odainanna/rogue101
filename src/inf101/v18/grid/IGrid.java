...
public interface IGrid<T> extends Iterable<T> {
   ...
   *
   * The function will be called with the (x,y) position of an element, and is expected to return
   * the element to place at that position. For example:
   *
   * <pre>
   ...
   *
   * Valid coordinates are 0 <= pos.getX() < getWidth(), 0 <= pos.getY() < getHeight().
   *
  ...
  /**
   * Create a parallel {@link Stream} with all the locations in this grid. <p> All locations
   * obtained through the stream are guaranteed to be valid according to {@link
   * #isValid(ILocation)}.
   *
   * @return A stream
   * @see {@link java.util.Collection#parallelStream()}
  ...
  /**
   * Iterate over all grid locations <p> See also {@link #iterator()} – using the grid directly in a
   * for-loop will iterate over the elements. <p> All locations obtained through the iterator are
   * guaranteed to be valid according to {@link #isValid(ILocation)}.
   *
  ...
  /**
   * Create a {@link Stream} with all the locations in this grid. <p> All locations obtained through
   * the stream are guaranteed to be valid according to {@link #isValid(ILocation)}.
   *
   * @return A stream
   */
